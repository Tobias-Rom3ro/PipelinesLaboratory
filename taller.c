...
void proceso_hijo1(int pipe_escritura, char *nombre_archivo) {
    ...
    // Agregar PID al mensaje
    char mensaje[BUFFER_SIZE + 50];
    snprintf(mensaje, sizeof(mensaje), "%s [pid Hijo1: %d]\n", contenido, getpid());
    
    printf("Hijo1 (%d): Enviando contenido al Padre...\n", getpid());
    write(pipe_escritura, mensaje, strlen(mensaje) + 1);
...
void proceso_nieto1(int pipe_lectura, int pipe_escritura) {
    ...
    printf("Nieto1 (%d): Recibí el mensaje, procesando...\n", getpid());
    
    // Agregar PID del Nieto1
    char mensaje[BUFFER_SIZE + 50];
    snprintf(mensaje, sizeof(mensaje), "%s [pid Nieto1: %d]\n", buffer, getpid());
...
void proceso_nieto2(int pipe_lectura, int pipe_escritura) {
    ...
    printf("Nieto2 (%d): Recibí el mensaje, procesando...\n", getpid());
    
    // Agregar PID del Nieto2
    char mensaje[BUFFER_SIZE + 50];
    snprintf(mensaje, sizeof(mensaje), "%s [pid Nieto2: %d]\n", buffer, getpid());
...
void proceso_hijo2(int pipe_lectura_padre, int pipe_escritura_padre) {
    ...
    printf("Hijo2 (%d): Recibí del Padre, enviando a Nieto2...\n", getpid());
    
    // Agregar PID del Hijo2 (primera vez)
    char mensaje[BUFFER_SIZE + 50];
    snprintf(mensaje, sizeof(mensaje), "%s [pid Hijo2: %d]\n", buffer, getpid());
    
    // 2. Enviar a Nieto2
    write(pipe_hijo2_nieto2[1], mensaje, strlen(mensaje) + 1);
    close(pipe_hijo2_nieto2[1]);
    
    // 3. Recibir de Nieto2
    read(pipe_nieto2_hijo2[0], buffer, BUFFER_SIZE);
    close(pipe_nieto2_hijo2[0]);
    printf("Hijo2 (%d): Recibí de Nieto2, enviando a Nieto1...\n", getpid());
    
    // Agregar PID del Hijo2 (segunda vez)
    snprintf(mensaje, sizeof(mensaje), "%s [pid Hijo2: %d]\n", buffer, getpid());
    
    // 4. Enviar a Nieto1
    write(pipe_hijo2_nieto1[1], mensaje, strlen(mensaje) + 1);
    close(pipe_hijo2_nieto1[1]);
    
    // 5. Recibir de Nieto1
    read(pipe_nieto1_hijo2[0], buffer, BUFFER_SIZE);
    close(pipe_nieto1_hijo2[0]);
    printf("Hijo2 (%d): Recibí de Nieto1, enviando al Padre...\n", getpid());
    
    // Agregar PID del Hijo2 (tercera vez)
    snprintf(mensaje, sizeof(mensaje), "%s [pid Hijo2: %d]\n", buffer, getpid());
    
    // 6. Enviar de vuelta al Padre
    write(pipe_escritura_padre, mensaje, strlen(mensaje) + 1);
...
int main(int argc, char *argv[]) {
    ...
    printf("=== INICIO DEL SISTEMA DE TUBERÍAS ===\n");
    printf("Padre (PID: %d)\n\n", getpid());
    ...
    close(pipe_hijo1_padre[0]);
    printf("Padre (%d): Recibí de Hijo1, enviando a Hijo2...\n", getpid());
    
    // Agregar PID del Padre (primera vez)
    char mensaje[BUFFER_SIZE + 50];
    snprintf(mensaje, sizeof(mensaje), "%s [pid Padre: %d]\n", buffer, getpid());
    
    // 2. Enviar a Hijo2
    write(pipe_padre_hijo2[1], mensaje, strlen(mensaje) + 1);
    close(pipe_padre_hijo2[1]);
    
    // 3. Recibir de Hijo2 (después de todo el proceso)
    read(pipe_hijo2_padre[0], buffer, BUFFER_SIZE);
    close(pipe_hijo2_padre[0]);
    printf("Padre (%d): Recibí de Hijo2 el mensaje final.\n\n", getpid());
    
    // Agregar PID del Padre (segunda vez)
    snprintf(mensaje, sizeof(mensaje), "%s [pid Padre: %d]\n", buffer, getpid());
